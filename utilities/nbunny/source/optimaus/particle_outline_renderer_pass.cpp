...
nbunny::ParticleOutlineRendererPass::ParticleOutlineRendererPass(GBuffer& o_buffer) :
	RendererPass(RENDERER_PASS_PARTICLE_OUTLINE), o_buffer(o_buffer)
{
	// Nothing.
}
	...
	sol::variadic_args args, sol::this_state S)
{
	...
	return std::make_shared<nbunny::ParticleOutlineRendererPass>(o_buffer);
}
