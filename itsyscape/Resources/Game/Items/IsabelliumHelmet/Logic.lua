--------------------------------------------------------------------------------
-- Resources/Game/Items/IsabelliumHelmet/Logic.lua
--
...
local IsabelliumHelmet = Class(Equipment)

function IsabelliumHelmet:onEquip(peep, item)
	local _, isabellium = self:getCalculatedBonuses(peep, item)
	isabellium:attach(peep)
end

function IsabelliumHelmet:getCalculatedBonuses(peep, item)
	local isabellium = Isabellium(item, Equipment.PLAYER_SLOT_HEAD, "IsabelliumHelmet")
