--------------------------------------------------------------------------------
-- Resources/Game/Items/IsabelliumBoots/Logic.lua
--
...
local IsabelliumBoots = Class(Equipment)

function IsabelliumBoots:onEquip(peep, item)
	local _, isabellium = self:getCalculatedBonuses(peep, item)
	isabellium:attach(peep)
end

function IsabelliumBoots:getCalculatedBonuses(peep, item)
	local isabellium = Isabellium(item, Equipment.PLAYER_SLOT_FEET, "IsabelliumBoots")
