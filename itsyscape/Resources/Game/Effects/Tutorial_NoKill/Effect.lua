...
function NoKill:applySelfToDamage(roll)
	...
	if status then
		roll:setMinHit(math.min(math.max(status.currentHitpoints - roll:getMinHitBoost() - 1, 0), roll:getMinHit()))
		roll:setMaxHit(math.min(math.max(status.currentHitpoints - roll:getMaxHitBoost() - 1, 0), roll:getMaxHit()))
