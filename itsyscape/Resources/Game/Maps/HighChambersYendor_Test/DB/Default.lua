local M = {}

M._MAP = ItsyScape.Resource.Map "HighChambersYendor_Test"

M["Bones_JustBones1"] = ItsyScape.Resource.MapObject.Unique()
do
	ItsyScape.Meta.MapObjectLocation {
		PositionX = 41.000000,
		PositionY = 2.000000,
		PositionZ = 19.000000,
		RotationX = 0.000000,
		RotationY = 0.000000,
		RotationZ = 0.000000,
		RotationW = 1.000000,
		ScaleX = 1.000000,
		ScaleY = 1.000000,
		ScaleZ = 1.000000,
		Name = "Bones_JustBones1",
		Map = M._MAP,
		Resource = M["Bones_JustBones1"]
	}

	ItsyScape.Meta.PropMapObject {
		Prop = ItsyScape.Resource.Prop "Bones_JustBones",
		MapObject = M["Bones_JustBones1"]
	}
end

M["Bones_JustBones2"] = ItsyScape.Resource.MapObject.Unique()
do
	ItsyScape.Meta.MapObjectLocation {
		PositionX = 41.500000,
		PositionY = 2.000000,
		PositionZ = 19.750000,
		RotationX = 0.000000,
		RotationY = 0.000000,
		RotationZ = 0.000000,
		RotationW = 1.000000,
		ScaleX = 1.000000,
		ScaleY = 1.000000,
		ScaleZ = 1.000000,
		Name = "Bones_JustBones2",
		Map = M._MAP,
		Resource = M["Bones_JustBones2"]
	}

	ItsyScape.Meta.PropMapObject {
		Prop = ItsyScape.Resource.Prop "Bones_JustBones",
		MapObject = M["Bones_JustBones2"]
	}
end

M["Bones_PileOfSkulls1"] = ItsyScape.Resource.MapObject.Unique()
do
	ItsyScape.Meta.MapObjectLocation {
		PositionX = 26.000000,
		PositionY = 2.000000,
		PositionZ = 18.000000,
		RotationX = 0.000000,
		RotationY = 0.382683,
		RotationZ = 0.000000,
		RotationW = 0.923880,
		ScaleX = 1.000000,
		ScaleY = 1.000000,
		ScaleZ = 1.000000,
		Name = "Bones_PileOfSkulls1",
		Map = M._MAP,
		Resource = M["Bones_PileOfSkulls1"]
	}

	ItsyScape.Meta.PropMapObject {
		Prop = ItsyScape.Resource.Prop "Bones_PileOfSkulls",
		MapObject = M["Bones_PileOfSkulls1"]
	}
end

M["Bones_Skull1"] = ItsyScape.Resource.MapObject.Unique()
do
	ItsyScape.Meta.MapObjectLocation {
		PositionX = 40.250000,
		PositionY = 2.000000,
		PositionZ = 19.500000,
		RotationX = 0.000000,
		RotationY = -0.382683,
		RotationZ = 0.000000,
		RotationW = 0.923880,
		ScaleX = 1.000000,
		ScaleY = 1.000000,
		ScaleZ = 1.000000,
		Name = "Bones_Skull1",
		Map = M._MAP,
		Resource = M["Bones_Skull1"]
	}

	ItsyScape.Meta.PropMapObject {
		Prop = ItsyScape.Resource.Prop "Bones_Skull",
		MapObject = M["Bones_Skull1"]
	}
end

M["Brazier_Isabelle1"] = ItsyScape.Resource.MapObject.Unique()
do
	ItsyScape.Meta.MapObjectLocation {
		PositionX = 41.000000,
		PositionY = 2.000000,
		PositionZ = 19.000000,
		RotationX = 0.000000,
		RotationY = 0.000000,
		RotationZ = 0.000000,
		RotationW = 1.000000,
		ScaleX = 1.000000,
		ScaleY = 1.000000,
		ScaleZ = 1.000000,
		Name = "Brazier_Isabelle1",
		Map = M._MAP,
		Resource = M["Brazier_Isabelle1"]
	}

	ItsyScape.Meta.PropMapObject {
		Prop = ItsyScape.Resource.Prop "Brazier_Isabelle",
		MapObject = M["Brazier_Isabelle1"]
	}
end

M["Brazier_Isabelle2"] = ItsyScape.Resource.MapObject.Unique()
do
	ItsyScape.Meta.MapObjectLocation {
		PositionX = 29.000000,
		PositionY = 2.000000,
		PositionZ = 19.000000,
		RotationX = 0.000000,
		RotationY = 0.000000,
		RotationZ = 0.000000,
		RotationW = 1.000000,
		ScaleX = 1.000000,
		ScaleY = 1.000000,
		ScaleZ = 1.000000,
		Name = "Brazier_Isabelle2",
		Map = M._MAP,
		Resource = M["Brazier_Isabelle2"]
	}

	ItsyScape.Meta.PropMapObject {
		Prop = ItsyScape.Resource.Prop "Brazier_Isabelle",
		MapObject = M["Brazier_Isabelle2"]
	}
end

M["Brazier_Isabelle3"] = ItsyScape.Resource.MapObject.Unique()
do
	ItsyScape.Meta.MapObjectLocation {
		PositionX = 29.000000,
		PositionY = 0.000000,
		PositionZ = 29.000000,
		RotationX = 0.000000,
		RotationY = 0.000000,
		RotationZ = 0.000000,
		RotationW = 1.000000,
		ScaleX = 1.000000,
		ScaleY = 1.000000,
		ScaleZ = 1.000000,
		Name = "Brazier_Isabelle3",
		Map = M._MAP,
		Resource = M["Brazier_Isabelle3"]
	}

	ItsyScape.Meta.PropMapObject {
		Prop = ItsyScape.Resource.Prop "Brazier_Isabelle",
		MapObject = M["Brazier_Isabelle3"]
	}
end

M["Brazier_Isabelle5"] = ItsyScape.Resource.MapObject.Unique()
do
	ItsyScape.Meta.MapObjectLocation {
		PositionX = 41.000000,
		PositionY = 0.000000,
		PositionZ = 29.000000,
		RotationX = 0.000000,
		RotationY = 0.000000,
		RotationZ = 0.000000,
		RotationW = 1.000000,
		ScaleX = 1.000000,
		ScaleY = 1.000000,
		ScaleZ = 1.000000,
		Name = "Brazier_Isabelle5",
		Map = M._MAP,
		Resource = M["Brazier_Isabelle5"]
	}

	ItsyScape.Meta.PropMapObject {
		Prop = ItsyScape.Resource.Prop "Brazier_Isabelle",
		MapObject = M["Brazier_Isabelle5"]
	}
end

M["Glyph1"] = ItsyScape.Resource.MapObject.Unique()
do
	ItsyScape.Meta.MapObjectLocation {
		PositionX = 35.000000,
		PositionY = 7.250000,
		PositionZ = 20.250000,
		RotationX = 0.000000,
		RotationY = 0.000000,
		RotationZ = 0.000000,
		RotationW = 1.000000,
		ScaleX = 5.250000,
		ScaleY = 5.250000,
		ScaleZ = 5.250000,
		Name = "Glyph1",
		Map = M._MAP,
		Resource = M["Glyph1"]
	}

	ItsyScape.Meta.PropMapObject {
		Prop = ItsyScape.Resource.Prop "Glyph",
		MapObject = M["Glyph1"]
	}
end

M["ParadoxGate1"] = ItsyScape.Resource.MapObject.Unique()
do
	ItsyScape.Meta.MapObjectLocation {
		PositionX = 20.754320,
		PositionY = 4.544352,
		PositionZ = 19.078873,
		RotationX = 0.056559,
		RotationY = -0.392320,
		RotationZ = -0.162307,
		RotationW = 0.903627,
		ScaleX = 1.000000,
		ScaleY = 1.000000,
		ScaleZ = 1.000000,
		Name = "ParadoxGate1",
		Map = M._MAP,
		Resource = M["ParadoxGate1"]
	}

	ItsyScape.Meta.PropMapObject {
		Prop = ItsyScape.Resource.Prop "ParadoxGate",
		MapObject = M["ParadoxGate1"]
	}
end

M["ParadoxGate2"] = ItsyScape.Resource.MapObject.Unique()
do
	ItsyScape.Meta.MapObjectLocation {
		PositionX = 49.000000,
		PositionY = 5.211580,
		PositionZ = 25.000000,
		RotationX = 0.051578,
		RotationY = -0.923545,
		RotationZ = 0.203191,
		RotationW = 0.321120,
		ScaleX = 1.000000,
		ScaleY = 1.000000,
		ScaleZ = 1.000000,
		Name = "ParadoxGate2",
		Map = M._MAP,
		Resource = M["ParadoxGate2"]
	}

	ItsyScape.Meta.PropMapObject {
		Prop = ItsyScape.Resource.Prop "ParadoxGate",
		MapObject = M["ParadoxGate2"]
	}
end

M["ProjectedGlyph1"] = ItsyScape.Resource.MapObject.Unique()
do
	ItsyScape.Meta.MapObjectLocation {
		PositionX = 35.000000,
		PositionY = 0.250000,
		PositionZ = 27.000000,
		RotationX = 0.000000,
		RotationY = 0.000000,
		RotationZ = 0.000000,
		RotationW = 1.000000,
		ScaleX = 1.000000,
		ScaleY = 1.000000,
		ScaleZ = 1.000000,
		Name = "ProjectedGlyph1",
		Map = M._MAP,
		Resource = M["ProjectedGlyph1"]
	}

	ItsyScape.Meta.PropMapObject {
		Prop = ItsyScape.Resource.Prop "ProjectedGlyph",
		MapObject = M["ProjectedGlyph1"]
	}
end

M["Schism1"] = ItsyScape.Resource.MapObject.Unique()
do
	ItsyScape.Meta.MapObjectLocation {
		PositionX = 35.000000,
		PositionY = 4.250000,
		PositionZ = 19.000000,
		RotationX = 0.000000,
		RotationY = 0.000000,
		RotationZ = 0.000000,
		RotationW = 1.000000,
		ScaleX = 0.500000,
		ScaleY = 0.500000,
		ScaleZ = 0.500000,
		Name = "Schism1",
		Map = M._MAP,
		Resource = M["Schism1"]
	}

	ItsyScape.Meta.PropMapObject {
		Prop = ItsyScape.Resource.Prop "Schism",
		MapObject = M["Schism1"]
	}
end

return M
