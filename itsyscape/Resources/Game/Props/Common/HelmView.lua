...
function HelmView:load()
	...
	self.staticNodes = self:loadAttachments(root, self.STATIC_ATTACHMENTS)

	self.wheelNode = SceneNode()
	self.wheelNode:setParent(root)
	self.wheelNodes = self:loadAttachments(self.wheelNode, self.WHEEL_ATTACHMENTS)
...
function HelmView:update(delta)
	...
	self:updateAttachments(self.staticNodes, self.STATIC_ATTACHMENTS)
	self:updateAttachments(self.wheelNodes, self.WHEEL_ATTACHMENTS)
end
