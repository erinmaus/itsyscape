...
GameProxy.spawnPlayer = Event.Create(PlayerProxy, function(event, gameManager, impl, game, ...)
	local player = impl(game, ...)
...
GameProxy.removePlayer = Event.Destroy(PlayerProxy, function(event, gameManager, impl, game, player)
	impl(game, player)
	gameManager:destroyInstance("ItsyScape.Game.Model.Player", player:getID())
end, Event.Argument("player", true))
GameProxy.removePlayer:link("onPlayerPoofed")
