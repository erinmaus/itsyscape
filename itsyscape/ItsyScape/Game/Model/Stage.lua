...
function Stage:new()
	self.onLoadMap = Callback(false)
	self.onUnloadMap = Callback(false)
	self.onMapModified = Callback(false)
	self.onMapMoved = Callback(false)
	self.onMapSkyUpdated = Callback(false)
	self.onMapMetaUpdated = Callback(false)
	self.onMapLinked = Callback(false)
	self.onMapUnlinked = Callback(false)
	self.onActorSpawned = Callback(false)
	self.onActorMoved = Callback(false)
	self.onActorKilled = Callback(false)
	self.onPropMoved = Callback(false)
	self.onDropItem = Callback(false)
	self.onTakeItem = Callback(false)
	self.onPropPlaced = Callback(false)
	self.onPropRemoved = Callback(false)
	self.onWaterFlood = Callback(false)
	self.onWaterDrain = Callback(false)
	self.onForecast = Callback(false)
	self.onStopForecast = Callback(false)
	self.onPlayMusic = Callback(false)
	self.onStopMusic = Callback(false)

	-- (projectileID, source, destination, time)
	self.onProjectile = Callback(false)

	-- (group, value [nil/Decoration])
	self.onDecorate = Callback(false)
	self.onUndecorate = Callback(false)
end
