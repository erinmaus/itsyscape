--------------------------------------------------------------------------------
-- ItsyScape/World/LinkedSmartPathFinder.lua
--
...
local LinkedSmartPathFinder = Class(PathFinder)

function LinkedSmartPathFinder:new(map, peep, t)
	t = t or {}
...
function LinkedSmartPathFinder:makeID(...)
	local current = self.edgeIDs

	for i = 1, select("#", ...) do
		local partialID = select(i, ...)

		local t = current[partialID]
		if not t then
			t = {}
			current[partialID] = t
		end
	...
	if not current[ID] then
		...
		current[ID] = self.edgeID
	end

	return current[ID]
end

function LinkedSmartPathFinder:getMap()
	return self.map
end

local D1 = 1
local D2 = math.sqrt(2)
function LinkedSmartPathFinder:makeEdge(i, j, parent, goal)
	...
	edge.score = parent.score + d

	return edge
end

function LinkedSmartPathFinder:makeActionEdge(i, j, parent, goal, prop, action)
	local edge = self:makeEdge(i, j, parent, goal)
	edge.prop = prop
	edge.action = action
	edge.nodeID = self:makeID(i, j, prop)

	return edge
end

function LinkedSmartPathFinder:getDoor(tile)
...
function LinkedSmartPathFinder:getNeighbors(edge, goal)
	local i = edge.i
	local j = edge.j
...
function LinkedSmartPathFinder:getScore(edge)
...
function LinkedSmartPathFinder:getLocation(edge)
	return edge
end

function LinkedSmartPathFinder:getDistance(a, b)
	local di = math.abs(a.i - b.i)
	local dj = math.abs(a.j - b.j)
	return di + dj
end

function LinkedSmartPathFinder:sameLocation(a, b)
	return a.i == b.i and a.j == b.j
end

function LinkedSmartPathFinder:materialize(edge)
	if edge.action then
...
function LinkedSmartPathFinder:getParent(edge)
	return edge.parent
end

return LinkedSmartPathFinder
